...
type DeleteTransactionReq struct {
	...
	AccountBookID uint `json:"account_book_id" form:"account_book_id" binding:"required"` // 账本id
}
...
type ModifyTransactionReq struct {
	ID            uint      `json:"id" form:"id" binding:"required"`                   //交易记录id
	AccountBookID uint      `json:"account_book_id" form:"account_book_id"`            // 对应的账本id
	Type          int       `json:"type" form:"type" binding:"required_with=Amount"`   // 类型:收入/支出,交易类型必须是0或者1
	Amount        float64   `json:"amount" form:"amount" binding:"required_with=Type"` // 交易数额,数值需要>0
